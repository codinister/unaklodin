...
export async function GET(req: NextRequest,{params}: {params: Promise<{value: string}>}) {
const values = (await params).value
  try {
    const data = await serverConfig.fetch(groq`
    *[_type in ['men', 'women', 'unisex']]{
    'type': _type, 
    ...
    `);
    return NextResponse.json(data.filter((v: ItemTypes) => Object.values(v.title).join('').toLowerCase().includes(values.toLowerCase())).slice(0,4));
  } catch (err) {
    console.log(err);
  }
}
